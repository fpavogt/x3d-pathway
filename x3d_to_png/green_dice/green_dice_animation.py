...
It sets up a typical Blender scene, imports an X3D mesh, applies some simple materials, and sets up a simple animation.
"""
...
bpy.data.scenes["Scene"].render.filepath = "/Volumes/Flash/green_dice/"						# Rendered frames output location.
...
# delete these and keep only the dice.

dice = bpy.data.objects["ShapeIndexedFaceSet"]  # "ShapeIndexedFaceSet" is the default name for imported X3D meshes.
dice.name = "Dice"								# We should rename to something more human friendly

spheres = bpy.data.objects["ShapeIndexedFaceSet.001"]
spheres.name = "Spheres"

# The "Spheres" exported from MayaVI have a low number of polygons.  To make them look a bit prettier we'll apply a quick subsurface modifier.
...
"""
For simplicity, we won't bother to add a material to the "Spheres" or "Plane" objects.
The default Cycles material is an off-white diffuse shader, perfect for both of these objects.
So we don't have to do anything for these two!
"""

## Dice material - a simple green diffuse material ##
# For the dice material we will use the default Cycles material but simply colour it green.
...
mat.node_tree.nodes["Diffuse BSDF"].inputs["Color"].default_value = (0.0, 1.0, 0.0, 1.0)		# Green colour
